...
`h` to show the complete key guide. The default Filtered view uses the existing
five-message log repeat summaries, hides transient/raw LED markers, and shows
LED state changes rather than unchanged repeats. Press `a` to toggle the All
view of every retained observation. LED timestamps are retained but hidden by
default; `t` shows or hides them.
...
background colour. `COLORFGBG`, when present, selects an initial light- or
dark-background palette; `c` cycles between those palettes and `m` selects
monochrome. Watch-mode reads check for `q` or an interrupt at most every 100 ms,
including while waiting for an L1 response, then close the device normally
without resetting its pipes solely because the user cancelled.
...
times. Each history accepts up to one million entries. Storage is allocated
for actual strings rather than a fixed byte allowance, and all histories are
discarded on exit. Long lines wrap within each pane with indented continuation
rows, and scrolling operates on those visual rows. The side-by-side LED pane
is capped at 129 terminal columns: 127 content columns plus its borders,
matching the longest line generated by the current LED table.
